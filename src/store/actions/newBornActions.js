...
export const fetchNewborns = () => async dispatch => {
  ...
  try {
    const newBornListresponse = await API.graphql(graphqlOperation(queries.listNewborns, { limit: 100 }));
    dispatch({ type: FETCH_NEWBORNS_SUCCESS, payload: newBornListresponse.data.listNewborns.items });
  } catch (error) {
...
export const fetchNewborn = newbornId => async dispatch => {
  ...
  try {
    const newBornResponse = await API.graphql(graphqlOperation(queries.getNewborn, { id: newbornId }));
    dispatch({ type: FETCH_NEWBORN_SUCCESS, payload: newBornResponse.data.getNewborn });
  } catch (error) {
...
export const fetchNewbornGenerations = () => async dispatch => {
  ...
  try {
    const newBornGenerationListResponse = await API.graphql(graphqlOperation(queries.listGenerations, { filter: { id: { eq: 'helo' } } }));
    dispatch({
...
export const fetchNewbornGeneration = generationId => async dispatch => {
  ...
  try {
    const newBornGenerationResponse = await API.graphql(graphqlOperation(queries.getGeneration, { id: generationId }));
    dispatch({
...
export const fetchNewbornEpisodes = () => async dispatch => {
  ...
  try {
    const newBornEpisodeListResponse = await API.graphql(graphqlOperation(queries.listEpisodes));
    dispatch({ type: FETCH_NEWBORN_EPISODES_SUCCESS, payload: newBornEpisodeListResponse.data });
  } catch (error) {
...
export const fetchNewbornEpisode = episodeId => async dispatch => {
  ...
  try {
    const newBornEpisodeResponse = await API.graphql(graphqlOperation(queries.getEpisode, { id: episodeId }));
    dispatch({ type: FETCH_NEWBORN_EPISODE_SUCCESS, payload: newBornEpisodeResponse.data.getEpisode.steps.items });
  } catch (error) {
